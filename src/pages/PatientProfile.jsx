                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-3 p-3 sm:p-4 bg-blue-50 border border-blue-100 transition-colors hover:bg-blue-100/50 cursor-pointer rounded-xl" onClick={() => handleAction('Communication Pipe (Email)')}>
                    ...
                    <div>
                      ...
                      <p className="text-[11px] font-black text-slate-800 tracking-tight truncate max-w-[150px]">{patientData.email}</p>
                    </div>
                  </div>
                </div>
              </Card>

              <Card hover={false} className="p-4 sm:p-5 border-none shadow-sm bg-slate-900 text-white overflow-hidden relative group">
                <div className="relative z-10 h-full flex flex-col">
                  <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-4">Financial Audit</h4>
                  <div className="bg-white/5 rounded-xl p-4 sm:p-6 border border-white/10 mb-4 sm:mb-6">
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 leading-none opacity-60">Verified Balance Due</p>
                    <h5 className="text-2xl font-black tracking-tighter text-white">{patientData.balance}</h5>
                  </div>
                  <Button 
                    variant="accent" 
            ...
            <Card hover={false} className="p-4 sm:p-5 border border-slate-100 shadow-none bg-white">
               ...
               <div className="space-y-3">
                  {[
                    ...
                    <div key={idx} className="flex flex-col sm:flex-row items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-100 hover:border-clinicPrimary/30 hover:bg-white transition-colors cursor-pointer group/item" onClick={() => navigate(item.type === 'Clinical' ? '/notes' : '/billing')}>
                       ...
                       <div className="flex items-center gap-4 mt-3 sm:mt-0">
                          <span className={`px-3 py-1 bg-white border border-slate-100 rounded-md text-[8px] font-black uppercase tracking-widest shadow-sm ${item.status === 'Verified' || item.status === 'Settled' ? 'text-emerald-500' : 'text-slate-400'}`}>{item.status}</span>
