...
const Analytics = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-10 px-4 sm:px-6 lg:px-8 py-8 animate-fade-in custom-scrollbar">
      <PageHeader 
        title="Intelligence Hub"
        subtitle="Audit operational trajectories and synchronize clinical performance benchmarks."
        icon={<FaChartPie />}
        actions={
          <div className="flex flex-col xl:flex-row items-center gap-4">
            <div className="flex bg-slate-100/50 p-1 rounded-2xl border border-slate-200 shadow-inner-soft">
               {['Monthly', 'Quarterly', 'Annual'].map(tab => (
                 <button 
                  key={tab}
                  onClick={() => setActiveTab(tab.toLowerCase())}
                  className={`px-6 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === tab.toLowerCase() ? 'bg-white shadow-premium text-clinicPrimary' : 'text-slate-400 hover:text-slate-600'}`}
                 >
                   {tab}
                 </button>
               ))}
            ...
            <div className="flex gap-2">
              <Button variant="secondary" className="h-10 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-none border-slate-200" leftIcon={<FaFilter />} onClick={() => setIsFilterModalOpen(true)}>Analytical Filter</Button>
              <Button variant="accent" className="h-10 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-google px-6" leftIcon={<FaDownload />} onClick={handleExport}>Export Report</Button>
            </div>
          </div>
        }
      />
